...
ChorusFX::ChorusFX()
    ...
    , lfo_depth_(0.5f)      // Default 50% depth (classic chorus)
    , lfo_freq_(0.25f)      // Default 0.25 Hz (slow, lush)
    , delay_ms_(20.0f)      // Default 20ms delay (thick chorus)
    , feedback_(0.2f)       // Default 20% feedback
    , wet_dry_(0.5f)        // Default 50/50 mix
    , bypassed_(true)       // Start bypassed (off by default)
    , lfo_depth_setting_value_(0.5f)
    , lfo_freq_setting_value_(0.25f)
    , delay_ms_setting_value_(20.0f)
    , feedback_setting_value_(0.2f)
...
void ChorusFX::Process(const float* const* in, float* const* out, size_t size) {
    ...
    for (size_t i = 0; i < size; i++) {
        ...
        float processed = chorus_.Process(in_sample);
        
        // Get stereo output
        float left = chorus_.GetLeft();
        float right = chorus_.GetRight();
        
        // Mix wet/dry
        float wet_l = left * wet_dry_;
        float wet_r = right * wet_dry_;
...
void ChorusFX::Update() {
    UpdateChorusParams();
}

void ChorusFX::UpdateUI() {
    // UI handled by settings system
}

void ChorusFX::HandleEncoder(int encoder, float delta) {
    (void)encoder;
    (void)delta;
}

void ChorusFX::HandleButton(int button, bool pressed) {
    (void)button;
    (void)pressed;
}

void ChorusFX::HandleJoystick(float x, float y) {
    (void)x;
    (void)y;
}

void ChorusFX::SetSampleRate(float sample_rate) {
...
void ChorusFX::SetBypass(bool bypass) {
    bypassed_ = bypass;
...
void ChorusFX::SetWetDry(float wet_dry) {
    wet_dry_ = wet_dry;
...
void ChorusFX::InitializeSettings() {
    // LFO Depth (float 0-1)
    ...
    settings_[2].max_value = 50.0f;
    settings_[2].step_size = 0.5f;
...
int ChorusFX::GetSettingCount() const {
    return 6;
}

const PluginSetting* ChorusFX::GetSetting(int index) const {
...
void ChorusFX::OnSettingChanged(int setting_index) {
...
void ChorusFX::UpdateChorusParams() {
    if (!initialized_) return;
    
    chorus_.SetLfoDepth(lfo_depth_);
    chorus_.SetLfoFreq(lfo_freq_);
    chorus_.SetDelayMs(delay_ms_);
    chorus_.SetFeedback(feedback_);
}

void ChorusFX::SaveState(void* buffer, size_t* size) const {
...
void ChorusFX::LoadState(const void* buffer, size_t size) {
...
size_t ChorusFX::GetStateSize() const {
