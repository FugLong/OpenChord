...
/**
 * BitcrusherFX - Bitcrusher/decimator effect plugin
 * 
 * Features:
 * - Bitcrush Factor (0-1, controls bit depth reduction)
 * - Downsample Factor (0-1, controls sample rate reduction)
 * - Wet/Dry mix
 * - Bypass
 */
class BitcrusherFX : public IEffectPlugin, public IPluginWithSettings {
public:
    BitcrusherFX();
    ~BitcrusherFX();
...
private:
    ...
    // Effect parameters
    float sample_rate_;
    float bitcrush_factor_;   // 0-1
    float downsample_factor_; // 0-1
    float wet_dry_;           // 0-1.0 (0=dry, 1=wet)
    bool bypassed_;
    
    // Settings storage
    float bitcrush_factor_setting_value_;
    float downsample_factor_setting_value_;
