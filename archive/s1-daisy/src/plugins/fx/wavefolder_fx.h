...
/**
 * WavefolderFX - Wavefolder effect plugin
 * 
 * Features:
 * - Gain (0-5, controls folding intensity)
 * - Offset (-1 to 1, for asymmetrical folding)
 * - Wet/Dry mix
 * - Bypass
 */
class WavefolderFX : public IEffectPlugin, public IPluginWithSettings {
public:
    WavefolderFX();
    ~WavefolderFX();
...
private:
    ...
    void UpdateWavefolderParams();
    
    // Effect parameters
    float sample_rate_;
    float gain_;             // 0-5
    float offset_;           // -1 to 1
    float wet_dry_;          // 0-1.0 (0=dry, 1=wet)
    bool bypassed_;
    
    // Settings storage
    float gain_setting_value_;
    float offset_setting_value_;
    ...
    // DaisySP wavefolder
    daisysp::Wavefolder wavefolder_;
    bool initialized_;
};
